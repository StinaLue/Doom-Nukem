...
/*   Updated: 2019/12/11 15:23:24 by afonck           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		draw_map(t_sdlmain *sdlmain, t_doom *doom, t_wall *walls, char *hud_flags)
{
	...
	if ((*hud_flags & ROT_MAP_SHOW) && (*hud_flags & FIX_MAP_SHOW) == 0)
		ret = draw_full_rotmap(doom->surfs.rot_mmap, &doom->player, walls, sdlmain->win_surf);
	else if ((*hud_flags & FIX_MAP_SHOW) && (*hud_flags & ROT_MAP_SHOW) == 0)
		ret = draw_full_fixedmap(doom->surfs.fixed_mmap, &doom->player, walls, sdlmain->win_surf);
