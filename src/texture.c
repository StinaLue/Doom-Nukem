...
/*   Updated: 2019/10/29 18:40:05 by sluetzen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_raycast *raycast, t_data *data)
{
	...
	while (y < raycast->end_line)
	{
		...
		{
			data->color =
				data->texture[data->tex_num][TEX_W * (data->tex_y) \
					+ data->tex_x];
			if (find_wall->side == 1)
			...
			t_raycast *raycast, t_data *data)
{
	...
	offset = (*data->map_ptr)[raycast->map_x][raycast->map_y] == 2 ? 4 : 0;
	if ((is_valid_wall((*data->map_ptr)[raycast->map_x][raycast->map_y])) == 1)
