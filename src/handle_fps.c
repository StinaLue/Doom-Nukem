...
/*   Updated: 2019/10/26 12:23:00 by sluetzen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		init_fps_surf(t_ttf *ttf, int current_fps, int *start_clock)
{
	*start_clock = SDL_GetTicks();
	...
	if ((ttf->surf_message = TTF_RenderText_Solid(ttf->font, ttf->fps, \
		ttf->color)) == NULL)
	{
...
void	update_fps(int *delta_clock, int *start_clock, int *current_fps, \
			int *wolf_fps)
{
