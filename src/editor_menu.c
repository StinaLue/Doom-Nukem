...
int	init_options_menu(t_editor *editor)
{
	...
	if (create_opt_str(editor, create_vec((editor->opt_surf->h) / 20, ((editor->opt_surf->h) / 20) * 4), 0, "CHOOSE HEIGHT") != 0)
		return (1);
	...
	if (create_opt_str(editor, create_vec((editor->opt_surf->h) / 20, ((editor->opt_surf->h) / 20) * 16), 2, "ENTER SOMETHING") != 0)
		return (1);
	if (create_opt_str(editor, create_vec((editor->opt_surf->w) / 2, ((editor->opt_surf->h) / 20) * 4), 3, "SET PLAYER POSITION") != 0)
		return (1);
