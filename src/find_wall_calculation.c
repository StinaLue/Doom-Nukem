...
/*   Updated: 2019/10/15 13:47:12 by afonck           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_wall_init(t_raycast const *raycast, t_wall_finding *find_wall)
{
	...
	if (raycast->dir_x < 0)
		find_wall->distance_x_side = find_wall->dist_x_to_x * (raycast->pos_x - raycast->map_x);
	else
		find_wall->distance_x_side = find_wall->dist_x_to_x * (raycast->map_x + 1 - raycast->pos_x);
	if (raycast->dir_y < 0)
		find_wall->distance_y_side = find_wall->dist_y_to_y * (raycast->pos_y - raycast->map_y);
	else
		find_wall->distance_y_side = find_wall->dist_y_to_y * (raycast->map_y + 1 - raycast->pos_y);
}
