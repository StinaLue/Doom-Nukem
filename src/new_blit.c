...
/*   Updated: 2020/01/15 16:16:46 by afonck           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void simple_blit(SDL_Surface *src, SDL_Surface *dst, SDL_Rect *dst_rect, Uint32 colorkey)
{
	t_vec screen_start;
	t_vec surf_start;
	Uint32 color;
	...
	while (surf_start.x < src->w && screen_start.x < dst->w)
	{
		...
		{
			color = ((Uint32 *)src->pixels)[surf_start.x + surf_start.y * src->w];
			if (color != colorkey)
				((Uint32 *)dst->pixels)[screen_start.x + screen_start.y * dst->w] = color;
			surf_start.y++;
			screen_start.y++;
		}
		surf_start.x++;
...
int	blit(SDL_Surface *src, SDL_Rect *src_rect, SDL_Surface *dst, SDL_Rect *dst_rect)
{
	...
	if (src == NULL || dst == NULL)
		return (error_return("passed null src or dst surf to blit function\n", NULL));
	if (SDL_GetColorKey(src, &colorkey) != 0)
		;
		//ft_dprintf(STDERR_FILENO, "Get color key in blit func error: %s\n", SDL_GetError());
	if (src_rect == NULL)
