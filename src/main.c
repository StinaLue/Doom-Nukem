...
/*   Updated: 2019/11/14 18:05:36 by sluetzen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_third_map(SDL_Surface *surf, t_player *player, t_wall *walls)
{
...
void	main_loop(t_doom *doom)
{
	...
	*/
	...
	if ((doom->sdl.second_map = SDL_CreateRGBSurface(0, SECOND_MAP_WIDTH, SECOND_MAP_HEIGHT, 32, 0, 0, 0, 0)) == NULL)
		printf("create surface error = %s\n", SDL_GetError());
	if ((doom->sdl.third_map = SDL_CreateRGBSurface(0, THIRD_MAP_WIDTH, THIRD_MAP_HEIGHT, 32, 0, 0, 0, 0)) == NULL)
		printf("create surface error = %s\n", SDL_GetError());
	...
	while (!doom->data.quit)
	{
		...
		ft_bzero(doom->sdl.third_map->pixels, doom->sdl.third_map->h * doom->sdl.third_map->pitch);
		draw_line(left_window_top, left_window_bottom, doom->sdl.first_map, 0xFFFFFF);
		draw_line(right_window_top, right_window_bottom, doom->sdl.second_map, 0xFFFFFF);
		...
		//print_third_map(doom->sdl.third_map, &doom->player, walls);
		...
		if ((SDL_BlitScaled(doom->sdl.second_map, NULL, doom->sdl.win_surf, &myrect_secondmap)) < 0)
			printf("BlitScale error = %s\n", SDL_GetError());
		if ((SDL_BlitScaled(doom->sdl.third_map, NULL, doom->sdl.win_surf, &myrect_thirdmap)) < 0)
			printf("BlitScale error = %s\n", SDL_GetError());
